...
# Create Salt perturbation more smooth
pert_salt = (vel_sigsbee[75:75+nz_, 150:150+n_] - vel) * mask[75:75+nz_, 150:150+n_]
pert_salt = gaussian_filter(pert_salt, sigma=4.0)
print("Perturbation statistics: max val = ", np.max(pert_salt), ", min val = ", np.min(pert_salt))
print("Total velocity statistics: max val = ", np.max(vel + pert_salt), ", min val = ", np.min(vel + pert_salt))
...
cb_ax = fig.add_axes([0.82, 0.35, 0.015, 0.3])
...
plt.show()
fig.savefig("G:/Research/Freq-Domain/Godzilla/Python/IntegralEquation/Fig/seg2022_vels_1.pdf", bbox_inches='tight', pad_inches=0)
