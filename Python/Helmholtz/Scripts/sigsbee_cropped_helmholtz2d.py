...
n = 101
vel_sigsbee = np.load("Python/Helmholtz/Data/sigsbee.npz")["arr_0"].T
vel_sigsbee *= 0.3048 * 0.001
Nx = vel_sigsbee.shape[0]
Nz = vel_sigsbee.shape[1]

vel_sigsbee_cropped = vel_sigsbee[150:150+n, 300:300+n]
vel1 = ndimage.gaussian_filter(input=vel_sigsbee_cropped, sigma=1) / 10   # in km/s
...
# np.savez("Python/Helmholtz/Data/sigsbee_cropped_sol.npz", x)
# x = np.load("Python/Helmholtz/Data/sigsbee_cropped_sol.npz")["arr_0"]
np.savez("Python/Helmholtz/Data/constant_sol.npz", x)
x = np.load("Python/Helmholtz/Data/constant_sol.npz")["arr_0"]
